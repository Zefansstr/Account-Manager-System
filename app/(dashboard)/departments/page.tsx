...
export default function DepartmentsPage() {
  ...
  const handleEdit = async () => {
    if (!selected) return;
    try {
      await updateDept.mutateAsync({ id: selected.id, data: formData });
      setIsEditOpen(false);
      setSelected(null);
      setFormData({ code: "", name: "", description: "" });
    } catch { /* toast in hook */ }
  ...
  const handleDelete = async () => {
    if (!selected) return;
    try {
      await deleteDept.mutateAsync(selected.id);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-[rgba(127,85,57,0.04)] dark:bg-[#101211] border border-[rgba(127,85,57,0.08)] dark:border-[#1f1f1f] rounded-2xl p-6">
      <div className="pb-5 border-b border-[rgba(30,30,30,0.12)] dark:border-[#1f1f1f] flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-medium text-[#1e1e1e] dark:text-white tracking-tight" style={{ fontFamily: "Inter, sans-serif" }}>Departments</h1>
          <p className="text-sm text-[rgba(127,85,57,0.62)] dark:text-gray-400 mt-1">Manage department entries and their account counts</p>
        </div>
        ...
        <table className="w-full table-fixed border-collapse">
            ...
            <tbody className="bg-white dark:bg-[#101211]">
              {loading ? (
                ...
                <tr><td colSpan={4} className="px-4 py-8 text-center text-[rgba(127,85,57,0.62)] dark:text-gray-400">No departments found</td></tr>
              ) : (
                departments.map((dept: Department) => (
                  <tr key={dept.id} className="border-b border-[rgba(127,85,57,0.08)] dark:border-[#1f1f1f] hover:bg-[rgba(245,237,230,0.5)] dark:hover:bg-[#1a1a1a] transition-colors">
                    <td className="px-4 py-3"><Badge variant="secondary" className="dark:bg-white/10 dark:text-gray-200">{dept.code}</Badge></td>
                    <td className="px-4 py-3 text-sm font-medium text-[#1e1e1e] dark:text-gray-200">{dept.name}</td>
                    <td className="px-4 py-3"><span className="inline-flex items-center rounded-full bg-[#7f5539] dark:bg-[#7f5539] px-3 py-1 text-sm font-semibold text-white">{dept.accountCount}</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Button variant="ghost" size="sm" className="hover:bg-[rgba(127,85,57,0.1)] hover:text-[#7f5539] dark:hover:bg-[rgba(127,85,57,0.2)] dark:hover:text-[#a06540]" onClick={() => { setSelected(dept); setFormData({ code: dept.code, name: dept.name, description: dept.description || "" }); setIsEditOpen(true); }}><Edit className="h-4 w-4" /></Button>
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="bg-white dark:bg-[#101211] border border-[rgba(127,85,57,0.2)] dark:border-[#1f1f1f]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label htmlFor="code" className="text-sm font-medium text-[#1e1e1e] dark:text-white">Code *</Label><Input id="code" className="h-9 text-sm text-[#1e1e1e] dark:text-gray-200" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="e.g. CRM_HOD" /></div>
            <div className="grid gap-2"><Label htmlFor="name" className="text-sm font-medium text-[#1e1e1e] dark:text-white">Department Name *</Label><Input id="name" className="h-9 text-sm text-[#1e1e1e] dark:text-gray-200" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. CRM HOD" /></div>
          ...
          <DialogFooter className="flex flex-row justify-end gap-3 pt-4 mt-1 border-t border-[rgba(127,85,57,0.12)]"><Button variant="outline" className="min-w-[88px]" onClick={() => setIsAddOpen(false)}>Cancel</Button><Button onClick={handleAdd} disabled={!formData.code || !formData.name || createDept.isPending} className="min-w-[140px] bg-[#7f5539] hover:bg-[#7f5539]/90 text-white">Add Department</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="bg-white dark:bg-[#101211] border border-[rgba(127,85,57,0.2)] dark:border-[#1f1f1f]">
          ...
          <DialogFooter className="flex flex-row justify-end gap-3 pt-4 mt-1 border-t border-[rgba(127,85,57,0.12)]"><Button variant="outline" className="min-w-[88px]" onClick={() => setIsEditOpen(false)}>Cancel</Button><Button onClick={handleEdit} disabled={!formData.code || !formData.name || updateDept.isPending} className="min-w-[160px] bg-[#7f5539] hover:bg-[#7f5539]/90 text-white">Update Department</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="bg-white dark:bg-[#101211] border border-[rgba(127,85,57,0.2)] dark:border-[#1f1f1f]">
          ...
          <DialogFooter className="flex flex-row justify-end gap-3 pt-4 mt-1 border-t border-[rgba(127,85,57,0.12)]"><Button variant="outline" className="min-w-[88px]" onClick={() => setIsDeleteOpen(false)}>Cancel</Button><Button variant="destructive" className="min-w-[88px]" onClick={handleDelete} disabled={deleteDept.isPending}>Delete</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
