        ...
        <table className="w-full table-fixed border-collapse">
            ...
            <tbody className="bg-white dark:bg-[#101211]">
              {loading ? (
                ...
                <tr><td colSpan={4} className="px-4 py-8 text-center text-[rgba(127,85,57,0.62)] dark:text-gray-400">No roles found</td></tr>
              ) : (
                roles.map((role: Role) => (
                  <tr key={role.id} className="border-b border-[rgba(127,85,57,0.08)] dark:border-[#1f1f1f] hover:bg-[rgba(245,237,230,0.5)] dark:hover:bg-[#1a1a1a] transition-colors">
                    <td className="px-4 py-3"><Badge variant="secondary" className="dark:bg-white/10 dark:text-gray-200">{role.code}</Badge></td>
                    <td className="px-4 py-3 text-sm font-medium text-[#1e1e1e] dark:text-gray-200">{role.name}</td>
                    <td className="px-4 py-3"><span className="inline-flex items-center rounded-full bg-[#7f5539] dark:bg-[#7f5539] px-3 py-1 text-sm font-semibold text-white">{role.accountCount}</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Button variant="ghost" size="sm" className="hover:bg-[rgba(127,85,57,0.1)] hover:text-[#7f5539] dark:hover:bg-[rgba(127,85,57,0.2)] dark:hover:text-[#a06540]" onClick={() => { setSelected(role); setFormData({ code: role.code, name: role.name, description: role.description || "" }); setIsEditOpen(true); }}><Edit className="h-4 w-4" /></Button>
