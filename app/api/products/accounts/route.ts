...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build base query - Use product_accounts table
    let query = supabase
      .from("product_accounts")
      .select(`
        *,
        product_applications:application_id(id, app_code, app_name),
        product_lines:line_id(id, line_code, line_name),
        product_departments:department_id(id, department_code, department_name),
        product_roles:role_id(id, role_code, role_name)
      `, { count: 'exact' });
    ...
    if (operatorId) {
      ...
      if (!isSuperAdmin && operator?.operator_role_id) {
        // Get permissions for "Accounts" menu in product-management module
        ...
        if (permissions) {
          ...
          if (hasApplicationFilter) {
            if (permissions.allowed_applications.length === 0) {
              ...
              // Use allowed applications directly (they should be product_applications IDs)
          ...
          if (hasLineFilter) {
            if (permissions.allowed_lines.length === 0) {
              ...
              // Use allowed lines directly (they should be product_lines IDs)
          ...
          if (hasDepartmentFilter) {
            if (permissions.allowed_departments.length === 0) {
              ...
              // Use allowed departments directly (they should be product_departments IDs)
    ...
    if (search) {
      // First get all matching role IDs from product_roles
    ...
    const transformed = data?.map((acc: any) => ({
      id: acc.id,
      application: acc.product_applications?.app_name || "-",
      applicationId: acc.application_id,
      line: acc.product_lines?.line_name || "-",
      lineId: acc.line_id,
      username: acc.username,
      password: acc.password,
      department: acc.product_departments?.department_name || "-",
      departmentId: acc.department_id,
      role: acc.product_roles?.role_name || "-",
      roleId: acc.role_id,
      remark: acc.remark,
      status: acc.status,
    }));
...
export async function POST(request: Request) {
  try {
    ...
    // Get operator from userId for audit logging
    const operatorStr = userId ? await supabase.from("operators").select("id").eq("id", userId).single() : null;
