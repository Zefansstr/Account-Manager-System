...
export default function ProductsApplicationsPage() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/products/applications");
  ...
  const handleAdd = async () => {
    try {
      ...
      if (res.ok) {
        toast.success(`Application "${formData.name}" created successfully!`);
        fetchApplications();
        setIsAddOpen(false);
      ...
      toast.error("An error occurred while creating application. Please try again.");
    }
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/products/applications/${selectedApp.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...formData,
      ...
      if (res.ok) {
        toast.success(`Application "${formData.name}" updated successfully!`);
        fetchApplications();
        setIsEditOpen(false);
        setSelectedApp(null);
        setFormData({ code: "", name: "", description: "" });
        ...
        if (result.error && (result.error.includes("duplicate key") || result.error?.includes("unique"))) {
          toast.error(`Code "${formData.code}" is already taken by another application. Please use a different code.`);
        } else {
          toast.error(result.error || "Failed to update application. Please try again.");
        }
        console.error("Error response:", result);
      }
    } catch (error) {
      console.error("Error updating application:", error);
      toast.error("An error occurred while updating application. Please try again.");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(result.error || "Failed to delete application. Please try again.");
      }
    } catch (error) {
      console.error("Error deleting application:", error);
      toast.error("An error occurred while deleting application. Please try again.");
    }
  };
    ...
    <div className="space-y-3">
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setIsAddOpen(false); setFormData({ code: "", name: "", description: "" }); }}>
              Cancel
            </Button>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setIsEditOpen(false); setSelectedApp(null); setFormData({ code: "", name: "", description: "" }); }}>
              Cancel
            </Button>
